...
def run_button_action(fig, case_name, iterations, initial_failures, load_generation_ratio, load_shed_constant, estimation_error, batch_size):
    ...
    # plt.show(block=False)
    return fig
def simple_run_button_action(fig, case_name, iterations, initial_failures, load_generation_ratio, load_shed_constant, estimation_error, batch_size, branch_data):
    """
    docstring
    TODO update this
    """
    ...
    if not os.path.exists(name + "_sm.mat"):
        ...
        print('Simulation with current settings already performed, loading matrices.')
    # generate the df
    states_df = generate_states_df(states_matrix_name=name + "_sm",initial_failure_table_name=name + "_if", clusters_matrix_name='cluster_branch_118', number_of_lines=186, use_test_cluster=False, output_df_name = "states_dataframe", use_simplified_df = False, states_matrix_folder = '', save_as_csv = False)
    #get the initial failures
    initial_failures = load_initial_failures(initial_failure_table_name= name + "_if", number_of_lines=186)
    #get other data
    ...
    #plt.plot([0.1, 0.2, 0.5, 0.7])
    # plt.show(block=False)
    return initial_failures, states_df, negativeOneIndices, mostFailureSimIndex, fig