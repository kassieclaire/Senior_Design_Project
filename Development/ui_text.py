...
# Tooltips
TOOLTIP_ITERATIONS = 'The number of individual simulations to run, each starting with different initial line failures and running until the cascading failure stops. Higher numbers of iterations will take longer to run, but will produce more reliable results.'
TOOLTIP_LOAD = 'This is the load-generation ratio for the grid. The load-generation ratio is the load on the grid as a percentage of the generation. 1.0 represents the sum of the loads being equivalent to the generation and 0.0 represents no load.'
TOOLTIP_INITIAL_FAILURES = 'This is the number of random line failures that occur at the start or each simulation iteration.'
TOOLTIP_LOAD_SHED_CONSTANT = 'This represents the constraints to which grid operators are held when making load-shedding decisions. Load-shedding is when a grid operator reduces or turns off electricity distribution to an area when the demand is higher than the grid can supply. 1.0 represents no load shedding permitted, while 0.0 represents no load shedding constraints.'
TOOLTIP_ERROR = 'This represents the estimation error operators have when determining the highest capacity of a line. 0.0 represents perfect knowledge of line capacities, 1.0 represents minimum knowledge of line capacities.'
...
# )
TEXT_ABOUT_INPUTS_POPUP = (
    'Iterations: ' + TOOLTIP_ITERATIONS,
    'Load: ' + TOOLTIP_LOAD,
    'Initial Failures: ' + TOOLTIP_INITIAL_FAILURES,
    'Operator Constraints: ' + TOOLTIP_LOAD_SHED_CONSTANT,
    'Line Capacity Uncertainty: ' + TOOLTIP_ERROR
)
